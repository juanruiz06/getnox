...
El equivalente de Android al AASA: hace que `https://getnox.es/p/…`, `/e/…`, `/v/…`,
`/u/…` y `/g/…` abran NOX en vez del navegador. Lo verifica el sistema por Digital Asset
...
En cuanto se suba el primer `.aab` a Play: Play Console → NOX → Release → Setup →
